...
from flat import create_median_flat
...
def run_reduction(data_dir):
    ...
    # Find files
    bias_list = sorted([str(f) for f in data_dir.glob('Bias_*.fit')])
    dark_list = sorted([str(f) for f in data_dir.glob('Dark_*.fit')])
    flat_list = sorted([str(f) for f in data_dir.glob('domeflat_*.fit')])
    science_files = sorted([str(f) for f in data_dir.glob('Cats_*.fit')])
    ...
    if reduced_files:
        ...
        positions = [(540, 540)]  # example position, adjust as needed
        aperture_radius = 15
        annulus_r_in = 8
        annulus_r_out = 12
